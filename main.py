...
def markdown_to_html(md_text: str) -> str:
    html_text = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'<a href="\2" target="_blank">\1</a>', md_text)
...
<style>
:root{--brand:#ff0f57; --brand-dark:#d10d47; --bg:#f5f7fb; --bot:#ffffff; --user:#ff0f57; --text:#20242b; --muted:#6b7280; --shadow:0 18px 40px rgba(0,0,0,0.12);}
...
<body>
<div class="widget">
<div class="header"><div class="logo">SC</div><div class="title">SKILL CAPITAL SUPPORT</div><div class="status"><span class="dot"></span>Online</div></div>
...
<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M2 21l21-9L2 3v7l15 2-15 2v7z" fill="currentColor"/></svg>
</button>
</div>
...
function addBubble(text, role='bot'){
  ...
  if(role==='bot'){ const av = document.createElement('div'); av.className='avatar'; av.textContent='SC'; row.appendChild(av); }
...
async function send(){
  const message=inputEl.value.trim(); if(!message) return; inputEl.value=''; addBubble(message,'user'); sendBtn.disabled=true;
  try{
    ...
    const data = await res.json(); addBubble(data.reply || '');
